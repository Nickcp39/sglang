#!/usr/bin/env bash
# Smoke test: build a tiny wheel, run rename_wheels.sh with SGL_KERNEL_CUDA_SUFFIX_OVERRIDE, verify metadata + filename + WHEEL Tag.
# Run from repo root on Linux/macOS (Git Bash): bash bug_20953_analysis/code/test_rename_wheels_smoke.sh
# Requires: Python with `wheel` (python3 or python), unzip; Git Bash on Windows OK.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SGL_KERNEL_DIR="${ROOT}/sgl-kernel"
WORKDIR=$(mktemp -d)
trap 'rm -rf "$WORKDIR"' EXIT

PY=""
try_py() {
  local c="$1"
  [[ -z "$c" ]] && return 1
  "$c" -c "import wheel" 2>/dev/null || return 1
  PY="$c"
  return 0
}
if [[ -n "${PYTHON_ROOT_PATH:-}" ]]; then
  try_py "${PYTHON_ROOT_PATH}/bin/python" || try_py "${PYTHON_ROOT_PATH}/python" || true
fi
if [[ -z "$PY" ]]; then
  command -v python >/dev/null 2>&1 && try_py "$(command -v python)" || true
fi
if [[ -z "$PY" ]]; then
  command -v python3 >/dev/null 2>&1 && try_py "$(command -v python3)" || true
fi
if [[ -z "$PY" ]]; then
  echo "Need python with \`wheel\` installed (conda/Git Bash: prefer \`python\` over Store \`python3\`)" >&2
  exit 1
fi

STAGE="${WORKDIR}/sgl_kernel-0.4.0"
mkdir -p "${STAGE}/sgl_kernel" "${STAGE}/sgl_kernel-0.4.0.dist-info"
echo 'x = 1' > "${STAGE}/sgl_kernel/__init__.py"
cat > "${STAGE}/sgl_kernel-0.4.0.dist-info/METADATA" << 'EOF'
Metadata-Version: 2.1
Name: sgl_kernel
Version: 0.4.0
EOF
cat > "${STAGE}/sgl_kernel-0.4.0.dist-info/WHEEL" << 'EOF'
Wheel-Version: 1.0
Generator: test
Root-Is-Purelib: false
Tag: cp310-abi3-linux_x86_64
EOF

DIST="${WORKDIR}/dist"
mkdir -p "$DIST"
(cd "$WORKDIR" && "$PY" -m wheel pack "${STAGE##*/}" --dest-dir "$DIST")

WHL=$(echo "$DIST"/*.whl)
[[ -f "$WHL" ]] || { echo "wheel pack failed" >&2; exit 1; }

cp -a "$SGL_KERNEL_DIR/rename_wheels.sh" "${WORKDIR}/rename_wheels.sh"
chmod +x "${WORKDIR}/rename_wheels.sh"

export SGL_KERNEL_CUDA_SUFFIX_OVERRIDE="+cu130"
mkdir -p "${WORKDIR}/dist"
mv "$WHL" "${WORKDIR}/dist/"
(cd "$WORKDIR" && ./rename_wheels.sh)

OUT=$(echo "${WORKDIR}/dist"/*.whl)
[[ -f "$OUT" ]] || { echo "expected output wheel" >&2; exit 1; }

echo "Output: $OUT"

unzip -p "$OUT" '*/METADATA' | grep -q '^Version: 0.4.0+cu130$' || \
  { echo "METADATA Version mismatch"; unzip -p "$OUT" '*/METADATA'; exit 1; }

unzip -p "$OUT" '*/WHEEL' | grep -q 'manylinux2014_x86_64' || \
  { echo "WHEEL Tag should contain manylinux2014_x86_64"; unzip -p "$OUT" '*/WHEEL'; exit 1; }

bash "${ROOT}/bug_20953_analysis/code/verify_wheel_metadata.sh" "$OUT"

echo "OK: rename_wheels (CUDA) smoke test passed."
